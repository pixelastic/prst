...
module.exports = {
   ...
   **/
  async displayRateLimit() {
    if (!config.get('args.debug')) {
      return;
    }
    const { remaining, reset } = await gitHubHelper.getRateLimit();
    const resetDifference = dayjs.unix(reset - dayjs().unix()).format('m');
    const resetDate = dayjs.unix(reset).format('hh:mma');
    console.info(
      `DEBUG: ${remaining} API credentials remaining. Reset in ${resetDifference}mn, at ${resetDate}`
    );
  },
